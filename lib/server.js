...
const BORDERS = 15;
const HEIGHT_PER_GRAPH = 80;
const PAGE_WIDTH = 600;
const DEFAULT_Y_PIXELS_PER_LABEL = 12;
...
Server.getTemplateReplacments = function() {
  if (replacements === undefined) {
    ...
    for (var i = 0; i < config.graphEntries.length; i++) {
      if (config.graphEntries[i].yPixelsPerLabel === undefined) {
        config.graphEntries[i].yPixelsPerLabel = DEFAULT_Y_PIXELS_PER_LABEL;
      }
      graphs = graphs + '                       { chartName: "chart' + i + '", retentionSeconds: "' +
                        config.graphEntries[i].retentionSeconds + '", name:"' +
                        config.graphEntries[i].name + '", yPixelsPerLabel: "' +
                        config.graphEntries[i].yPixelsPerLabel + '" }';
